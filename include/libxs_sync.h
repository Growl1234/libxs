...
#else
# define LIBXS_ATOMIC_LOCKORDER LIBXS_ATOMIC_SEQ_CST
#endif

#define LIBXS_NONATOMIC_LOCKTYPE LIBXS_ATOMIC_LOCKTYPE
#define LIBXS_NONATOMIC_LOAD(SRC_PTR, KIND) (*(SRC_PTR))
#define LIBXS_NONATOMIC_STORE(DST_PTR, VALUE, KIND) do { LIBXS_UNUSED(KIND); *(DST_PTR) = (VALUE); } while(0)
...
#define LIBXS_NONATOMIC_CMPSWP(DST_PTR, OLDVAL, NEWVAL, KIND) (*(DST_PTR) == (OLDVAL) && (*(DST_PTR) = (NEWVAL), 1))
#define LIBXS_NONATOMIC_TRYLOCK(DST_PTR, KIND) LIBXS_NONATOMIC_CMPSWP(DST_PTR, 0, 1, KIND)
#define LIBXS_NONATOMIC_ACQUIRE(DST_PTR, NPAUSE, KIND) do { LIBXS_UNUSED(NPAUSE); \
          LIBXS_ASSERT_MSG(0 == *(DST_PTR), "LIBXS_NONATOMIC_ACQUIRE"); LIBXS_NONATOMIC_STORE(DST_PTR, 1, KIND); \
          LIBXS_ASSERT_MSG(0 != *(DST_PTR), "LIBXS_NONATOMIC_ACQUIRE"); } while(0)
#define LIBXS_NONATOMIC_RELEASE(DST_PTR, KIND) do { LIBXS_UNUSED(DST_PTR); LIBXS_UNUSED(KIND); \
          LIBXS_ASSERT_MSG(0 != *(DST_PTR), "LIBXS_NONATOMIC_RELEASE"); LIBXS_NONATOMIC_STORE(DST_PTR, 0, KIND); \
          LIBXS_ASSERT_MSG(0 == *(DST_PTR), "LIBXS_NONATOMIC_RELEASE"); } while(0)
...
#   endif
#   if defined(__PGI)
#     define LIBXS_ATOMIC_RELEASE(DST_PTR, KIND) do { LIBXS_ASSERT_MSG(0 != *(DST_PTR), "LIBXS_ATOMIC_RELEASE"); \
              LIBXS_ATOMIC_STORE_ZERO8(DST_PTR, KIND); } while(0) /* matches bit-width of LIBXS_ATOMIC_LOCKTYPE */
#   else
#     define LIBXS_ATOMIC_RELEASE(DST_PTR, KIND) do { LIBXS_ASSERT_MSG(0 != *(DST_PTR), "LIBXS_ATOMIC_RELEASE"); \
              __atomic_clear(DST_PTR, KIND); } while(0)
#   endif
...
#   endif
#   define LIBXS_ATOMIC_RELEASE(DST_PTR, KIND) do { LIBXS_ASSERT_MSG(0 != *(DST_PTR), "LIBXS_ATOMIC_RELEASE"); \
            __sync_lock_release(DST_PTR); } while(0)
#   define LIBXS_ATOMIC_SYNC(KIND) __sync_synchronize()
# endif
# if defined(LIBXS_ATOMIC_ZERO_STORE)
...
# endif
# define LIBXS_ATOMIC_FETCH_OR(DST_PTR, VALUE, KIND) InterlockedOr((volatile LONG*)(DST_PTR), (LONG)VALUE)
...
# define LIBXS_ATOMIC_SUB_FETCH(DST_PTR, VALUE, KIND) ((LONG)LIBXS_ATOMIC_FETCH_SUB(DST_PTR, VALUE, KIND) - ((LONG)VALUE))
...
# define LIBXS_ATOMIC_FETCH_ADD(DST_PTR, VALUE, KIND) InterlockedExchangeAdd((volatile LONG*)(DST_PTR), (LONG)VALUE)
# define LIBXS_ATOMIC_FETCH_ADD16(DST_PTR, VALUE, KIND) _InterlockedExchangeAdd16((volatile SHORT*)(DST_PTR), (SHORT)VALUE)
# define LIBXS_ATOMIC_FETCH_ADD64(DST_PTR, VALUE, KIND) InterlockedExchangeAdd64((volatile LONGLONG*)(DST_PTR), (LONGLONG)VALUE)
...
/** Utility function to receive a rank ID of the calling process. */
