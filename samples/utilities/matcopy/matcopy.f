...
!=======================================================================!
     ...
     &                        ptr => libxs_ptr
        ...
        INTEGER(LIBXS_BLASINT_KIND) :: m, n, ldi, ldo, h, i, j
     ...
     &    " size=", INT(k,8) * INT(nbytes,8) / ISHFT(1, 20), "MB"
        ...
        DO r = 1, nrepeat
          IF (0.NE.zero) THEN
            ...
            DO h = 1, k
              !CALL libxs_xmatcopy(bn(:,:,h), m=m,n=n, ldi=ldi,ldo=ldo)
            ...
            IF (0.NE.check) THEN
              WRITE(*, "(A,F10.1,A,1A,F10.1,A)") "LIBXS (zero):", 1D3 &
     ...
     &          / (REAL(ISHFT(1, 20), 8) * d), " MB/s"
          ...
          IF ((0.EQ.zero).OR.(1.LT.zero)) THEN
            start = libxs_timer_tick()
            DO h = 1, k
              !CALL libxs_xmatcopy(bn(:,:,h), an(:,:,h), m,n, ldi,ldo)
     ...
     &          S, m, n, ldi, ldo)
            ...
            IF (0.NE.check) THEN
              WRITE(*, "(A,F10.1,A,1A,F10.1,A)") "LIBXS (copy):", 1D3 &
     &          * d, " ms", CHAR(9), REAL(2 * k, 8) * REAL(nbytes, 8)   &
     &          / (REAL(ISHFT(1, 20), 8) * d), " MB/s"
            END IF
          END IF
          ! skip non-LIBXS measurements
          IF (0.EQ.check) CYCLE
     ...
     &        / (REAL(ISHFT(1, 20), 8) * duration(4)), " MB/s"
        ...
        ELSE
          SELECT CASE (error)
            CASE (1)
              WRITE(*, "(A)") "Error: LIBXS-zero failed!"
            CASE (2)
              WRITE(*, "(A)") "Error: LIBXS-copy failed!"
            CASE (3)
      ...
      CONTAINS
        PURE REAL(T) FUNCTION initial_value(i, j, m)
          INTEGER(LIBXS_BLASINT_KIND), INTENT(IN) :: i, j, m
        ...
        PURE REAL(T) FUNCTION diff(check, m, mat, ref)
          ...
          INTEGER(LIBXS_BLASINT_KIND), INTENT(IN) :: m
          INTEGER(LIBXS_BLASINT_KIND) :: h, i, j
          diff = Z
          IF (0.NE.check) THEN
