...
/* bypass LIBXS_APIVAR_DECL_ALIGNED: its empty macro argument trips a C90 */
/* pedantic warning (C90 6.8.3) */
#define OZAKI_APIVAR_DEF(DECL, VIS) \
  LIBXS_ALIGNED(LIBXS_APIKIND_COMMON(OZAKI_BUILD_KIND) LIBXS_APIKIND_VIS(VIS, OZAKI_BUILD_KIND) DECL, LIBXS_ALIGNMENT)
#define OZAKI_APIVAR_PUBLIC_DEF(DECL) OZAKI_APIVAR_DEF(DECL, EXPORT)
#define OZAKI_APIVAR_PRIVATE_DEF(DECL) OZAKI_APIVAR_DEF(DECL, INTERN)
...
/* GEMM_IS_DOUBLE expands to 1 for double, 0 for float */
/* token-pasting yields a preprocessor-evaluable constant, which is needed */
/* because LIBXS_DATATYPE enum values are invisible to #if */
