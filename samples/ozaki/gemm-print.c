...
LIBXS_API void print_gemm(FILE* ostream, int compact,
  ...
  const GEMM_REAL_TYPE*  beta, GEMM_REAL_TYPE* c, const GEMM_INT_TYPE* ldc)
{
  ...
  if (1 == compact || 0 > compact) { /* compact (one line) */
    fprintf(ostream, "%s(trans=%c%c, mnk=%llix%llix%lli, ld=%llix%llix%lli, alpha=%g, beta=%g)\n",
      ...
      (long long int)*lda, (long long int)*ldb, (long long int)*ldc, *alpha, *beta);
  }
...
LIBXS_API void print_diff(FILE* ostream, const libxs_matdiff_t* diff)
{
  ...
  if (1E-6 <= epsilon) {
    fprintf(ostream, "GEMM: ncalls=%i linf=%.17g linf_rel=%.17g l2_rel=%.17g eps=%f rsq=%f -> %g != %g\n",
      diff->r, diff->linf_abs, diff->linf_rel, diff->l2_rel, epsilon, diff->rsq,
      diff->v_ref, diff->v_tst);
  }
  else {
    fprintf(ostream, "GEMM: ncalls=%i linf=%.17g linf_rel=%.17g l2_rel=%.17g eps=%f rsq=%f\n",
      diff->r, diff->linf_abs, diff->linf_rel, diff->l2_rel, epsilon, diff->rsq);
  }
}
