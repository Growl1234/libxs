  ...
  unsigned int diff_stat, libxs_matdiff_t* diff)
{
  ...
  const GEMM_INT_TYPE K_pad = ((K + BLOCK_K - 1) / BLOCK_K) * BLOCK_K;
  ...
  a_slices = (int8_t*)libxs_malloc(gemm_pool,
    (size_t)nslices * M * K_pad, 0);
  b_slices = (int8_t*)libxs_malloc(gemm_pool,
    (size_t)nslices * N * K_pad, 0);
  expa_raw = (int16_t*)libxs_malloc(gemm_pool,
    (size_t)M * sizeof(int16_t), 0);
  expb_raw = (int16_t*)libxs_malloc(gemm_pool,
    (size_t)N * sizeof(int16_t), 0);
  expa_fp = (double*)libxs_malloc(gemm_pool,
    (size_t)M * sizeof(double), 0);
  expb_fp = (double*)libxs_malloc(gemm_pool,
      ...
      for (kk = 0; kk < K; ++kk) {
        ...
        if (0 != mt) {
          ...
          for (ss = 0; ss < nslices; ++ss) {
            a_slices[(long)ss * M * K_pad + (long)row * K_pad + kk] = tmp[ss];
          }
        }
      }
    }

    /* Phase 2: preprocess all columns of B (full K dimension) */
#if defined(_OPENMP)
#   pragma omp for schedule(static)
    ...
    for (col = 0; col < N; ++col) {
      ...
      for (kk = 0; kk < K; ++kk) {
        int16_t e; uint64_t mt; int sign;
        sign = ozaki_extract_ieee(b[LIBXS_INDEX(tb, *ldb, kk, col)],
          &e, &mt);
        if (0 != mt) {
          ...
          LIBXS_PRAGMA_LOOP_COUNT(1, MAX_NSLICES, NSLICES_DEFAULT)
          for (ss = 0; ss < nslices; ++ss) {
            b_slices[(long)ss * N * K_pad + (long)col * K_pad + kk] = tmp[ss];
          }
        }
      }
    }
...
#endif
      for (row = 0; row < M; ++row) {
        GEMM_REAL_TYPE ref_blk[BLOCK_K]; GEMM_REAL_TYPE recon_blk[BLOCK_K];
        GEMM_INT_TYPE kk;
        ...
        for (kk = 0; kk < K; ++kk) {
          ...
          for (si = 0; si < nslices; ++si) {
            tmp[si] = a_slices[(long)si * M * K_pad + (long)row * K_pad + kk];
          }
          arecon = reconstruct_from_digits(tmp, exp_base, slice_low_bit);
          ref_blk[kk % BLOCK_K] = a[LIBXS_INDEX(ta, *lda, row, kk)];
          recon_blk[kk % BLOCK_K] = (GEMM_REAL_TYPE)arecon;
          if (BLOCK_K - 1 == (kk % BLOCK_K) || kk == K - 1) {
            GEMM_INT_TYPE bsize = (kk % BLOCK_K) + 1;
            libxs_matdiff_t bd;
            const int ild = 1, itd = 1;
            if (EXIT_SUCCESS == libxs_matdiff(&bd,
              LIBXS_DATATYPE(GEMM_REAL_TYPE), bsize, 1,
              ref_blk, recon_blk, &ild, &itd))
            {
...
#     pragma omp for schedule(static)
      ...
      for (col = 0; col < N; ++col) {
        ...
        for (kk = 0; kk < K; ++kk) {
          ...
          LIBXS_PRAGMA_LOOP_COUNT(1, MAX_NSLICES, NSLICES_DEFAULT)
          for (si = 0; si < nslices; ++si) {
            tmp[si] = b_slices[(long)si * N * K_pad + (long)col * K_pad + kk];
          }
          brecon = reconstruct_from_digits(tmp, exp_base, slice_low_bit);
          ref_blk[kk % BLOCK_K] = b[LIBXS_INDEX(tb, *ldb, kk, col)];
          recon_blk[kk % BLOCK_K] = (GEMM_REAL_TYPE)brecon;
          if (BLOCK_K - 1 == (kk % BLOCK_K) || kk == K - 1) {
            GEMM_INT_TYPE bsize = (kk % BLOCK_K) + 1;
            libxs_matdiff_t bd;
            const int ild = 1, itd = 1;
...
#   pragma omp for LIBXS_OPENMP_COLLAPSE(2) schedule(static)
      ...
      for (ib = 0; ib < M; ib += BLOCK_M) {
        ...
        if (NULL != ref_panel) {
          ...
          ozaki_scale_block_beta(cb, ldcv, iblk, jblk, beta,
            ref_panel + (jbi * nblk_m + ibi) * BLOCK_M * BLOCK_N, 1);
        }
        else {
     ...
     * Triangular iteration and mirror-pair symmetrization match the old
     * panel-batched path. */
    LIBXS_PRAGMA_LOOP_COUNT(1, MAX_NSLICES, NSLICES_DEFAULT)
    for (slice_a = 0; slice_a < nslices && slice_a <= cutoff; ++slice_a) {
      ...
      for (slice_b = sb_start; slice_b < sb_end; ++slice_b) {
        const double pair_scale = (*alpha) * pow2_low[slice_a]
          * pow2_low[slice_b];
...
#     pragma omp for LIBXS_OPENMP_COLLAPSE(2) schedule(static)
      ...
      for (jb = 0; jb < N; jb += BLOCK_N) {
        for (ib = 0; ib < M; ib += BLOCK_M) {
          ...
          const GEMM_INT_TYPE jblk = LIBXS_MIN(BLOCK_N, N - jb);
          GEMM_REAL_TYPE *const cb = c + jb * ldcv + ib;
          GEMM_REAL_TYPE *const ref_blk = ref_panel
            + (jbi * nblk_m + ibi) * BLOCK_M * BLOCK_N;
          ...
          ozaki_accumulate_block_diff(&tdiff[tid], ref_blk, cb,
            iblk, jblk, mref, ldcv);
        }
      }
    }
  ...
  libxs_free(a_slices); libxs_free(b_slices);
  libxs_free(expa_raw); libxs_free(expb_raw);
  libxs_free(expa_fp); libxs_free(expb_fp);
...
# pragma omp parallel for LIBXS_OPENMP_COLLAPSE(2) schedule(static)
  ...
  for (ki = 0; ki < nkb; ++ki) {
    for (ib_idx = 0; ib_idx < nblk; ++ib_idx) {
      ...
      int mi;
      preprocess_rows(a, (GEMM_INT_TYPE)ld, trans,
        (GEMM_INT_TYPE)dim, (GEMM_INT_TYPE)K,
        (GEMM_INT_TYPE)ib, (GEMM_INT_TYPE)kb, iblk, kblk,
        expa_row, ak_tile);
      for (mi = 0; mi < brc; ++mi) {
        memcpy((char*)slices + ((long)panel * brc + mi) * nslices_p * bk,
          &ak_tile[mi][0][0], (size_t)nslices_p * bk);
      }
      memcpy((short*)exp + (long)panel * brc, expa_row,
...
# pragma omp parallel for LIBXS_OPENMP_COLLAPSE(2) schedule(static)
  ...
  for (ki = 0; ki < nkb; ++ki) {
    for (jb_idx = 0; jb_idx < nblk; ++jb_idx) {
      ...
      int nj, s, kk;
      preprocess_cols(b, (GEMM_INT_TYPE)ld, trans,
        (GEMM_INT_TYPE)dim, (GEMM_INT_TYPE)K,
        (GEMM_INT_TYPE)jb, (GEMM_INT_TYPE)kb, jblk, kblk,
        expb_col, bk_tile);
      if (0 == use_xmx_p) {
        for (nj = 0; nj < brc; ++nj) {
          memcpy((char*)slices + ((long)panel * brc + nj) * nslices_p * bk,
            &bk_tile[nj][0][0], (size_t)nslices_p * bk);
        }
      }
      else {
      ...
      memcpy((short*)exp + (long)panel * brc, expb_col,
        (size_t)brc * sizeof(short));
    }
  }
}
