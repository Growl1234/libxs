...
LIBXS_APIVAR_PRIVATE_DEF(int gemm_ozcutoff);
  ...
  const GEMM_REAL_TYPE*  beta, GEMM_REAL_TYPE* c, const GEMM_INT_TYPE* ldc)
{
  ...
  if (0 == gemm_initialized) {
    ...
    if (0 == gemm_initialized) {
      ...
      const char *const gemm_ozcutoff_env = getenv("GEMM_OZCUTOFF");
      ...
      gemm_ozcutoff = (NULL == gemm_ozcutoff_env ? -1/*exact*/ : atoi(gemm_ozcutoff_env));
