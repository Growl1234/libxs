  ...
  unsigned int diff_stat, libxs_matdiff_t* diff)
{
  ...
  a_res = (oz2_res_t*)libxs_malloc(gemm_pool,
    (size_t)nprimes * M * K_pad, 0);
  b_res = (oz2_res_t*)libxs_malloc(gemm_pool,
    (size_t)nprimes * N * K_pad, 0);
  ...
  memset(a_res, 0, (size_t)nprimes * M * K_pad);
  memset(b_res, 0, (size_t)nprimes * N * K_pad);
...
#     pragma omp for schedule(static)
      ...
      for (row = 0; row < M; ++row) {
        ...
        for (kk = 0; kk < K; ++kk) {
          ...
          double arecon;
          for (pidx = 0; pidx < nprimes; ++pidx) {
            const oz2_res_t rv = a_res[(long)pidx * M * K_pad
              + (long)row * K_pad + kk];
          ...
          if (BLOCK_K - 1 == (kk % BLOCK_K) || kk == K - 1) {
            const GEMM_INT_TYPE bsize = (kk % BLOCK_K) + 1;
            libxs_matdiff_t bd;
...
#     pragma omp for schedule(static)
      ...
      for (col = 0; col < N; ++col) {
        ...
        for (kk = 0; kk < K; ++kk) {
          ...
          double brecon;
          for (pidx = 0; pidx < nprimes; ++pidx) {
            const oz2_res_t rv = b_res[(long)pidx * N * K_pad
              + (long)col * K_pad + kk];
            tmp[pidx] = (uint8_t)(rv < 0 ? -rv : rv);
          }
          mant_recon = oz2_reconstruct_mantissa(tmp, garner_inv, nprimes);
...
#   pragma omp for LIBXS_OPENMP_COLLAPSE(2) schedule(static)
     ...
     * then CRT reconstruct to signed double, scale, and accumulate. */
#if defined(_OPENMP)
#   pragma omp for LIBXS_OPENMP_COLLAPSE(2) schedule(static)
#endif
    for (jb = 0; jb < N; jb += BLOCK_N) {
