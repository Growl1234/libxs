...
def select_hdf5_volume(dataset, layout, event, channel, reshape):
    ...
    if reshape is not None:
        ...
        if array.size != elements:
            raise ValueError(
                "selected HDF5 volume has %d values, expected %d"
                % (array.size, elements)
            )
...
def load_hdf5_volume(path, dataset_name, layout, event, channel, reshape):
    ...
    if array.ndim != 3:
        raise ValueError(
            "selected HDF5 volume has shape %s, expected 3D" % (array.shape,)
        )
    depth, height, width = (
        int(array.shape[0]),
        int(array.shape[1]),
...
def stratify(libxs, curve, depth, height, width, volume, frame="compact"):
    ...
    if curve == "hilbert":
        stratify_fn = (
            libxs.libxs_stratify_hilbert_bits
            if use_bits
        ...
        stratify_fn = (
            libxs.libxs_stratify_morton_bits
            if use_bits
    ...
    for z_coord in range(depth):
        for y_coord in range(height):
            for x_coord in range(width):
                ...
                if use_bits:
                    result = stratify_fn(
                        src_coords, 3, src_bits, dst_coords, 2, dst_bits
                    )
                else:
