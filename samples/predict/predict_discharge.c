...
int main(int argc, char* argv[])
{
  ...
  if (NULL == filename) {
    fprintf(stdout,
      ...
      "  Predicts next %d days from previous %d days.\n"
    ...
    libxs_predict_t* model = libxs_predict_create(WINDOW + 1, HORIZON);
    fprintf(stdout, "Loaded %d daily discharge values from %s\n", total, filename);
    fprintf(stdout, "Window=%d (+day-of-year), Horizon=%d, Train=%d, Test=%d\n",
      WINDOW, HORIZON, train_end - WINDOW, total - train_end);
    if (NULL != model) {
      double inputs[WINDOW + 1], outputs[HORIZON];
      int t;
