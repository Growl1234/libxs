...
int main(int argc, char* argv[])
{
  ...
  const libxs_trsm_descriptor* desc8 = NULL;
#ifdef TEST_SINGLE
  unsigned int typesize4 = 4;
  const libxs_trsm_descriptor* desc4 = NULL;
#endif
#ifdef USE_XSMM_GENERATED
  libxs_descriptor_blob blob;
  libxs_trsm_xfunction mykernel = NULL;
#endif
  ...
  nmats = LIBXS_MAX(VLENS,nmat - (nmat%VLENS));
  nmatd = LIBXS_MAX(VLEND,nmat - (nmat%VLEND));
  nmat = LIBXS_MAX(nmats,nmatd);
  ...
  desc8 = libxs_trsm_descriptor_init(&blob, typesize8, m, n, lda, ldb, &dalpha, trans, diag, side, uplo, layout);
#ifdef TEST_SINGLE
  desc4 = libxs_trsm_descriptor_init(&blob, typesize4, m, n, lda, ldb, &salpha, trans, diag, side, uplo, layout);
#endif
#ifdef USE_XSMM_GENERATED
  printf("calling libxs_dispatch_trsm: typesize8=%u\n",typesize8);
  mykernel = libxs_dispatch_trsm(desc8);
  printf("done calling libxs_dispatch_trsm: typesize8=%u\n",typesize8);
  if ( mykernel == NULL ) printf("R8 Kernel after the create call is null\n");
#ifdef TEST_SINGLE
  mykernel = libxs_dispatch_trsm(desc4);
...
# include <mkl.h>
  ...
  for ( i = 0 , num = 0; i < (int)nmatd; i+= (int)VLEND, num++ )
  {
     ...
     timer += libxs_timer_duration(l_start,l_end);
#endif
  }
  }
  timer /= ((double)ntest);
