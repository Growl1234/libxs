     ...
     for ( j = 1; j <= col; j++ ) {
        for ( i = 1; i <= ld; i++ ) {
           for ( k = 1; k <= VLEN; k++ ) {
              ...
              if ( k == 1 )
#endif
              if ( i <= row ) dtmp = 1.0 - 2.0*libxs_rng_f64();
              else            dtmp = -99.9;
              matrix [ address ] = dtmp;
           }
        }
...
int main(int argc, char* argv[])
{
  ...
  const libxs_getrf_descriptor* desc8 = NULL;
#ifdef TEST_SINGLE
  ...
  libxs_getrf_xfunction mykernel = NULL;
#endif
  ...
  m = LIBXS_MAX(m,1);
  n = LIBXS_MAX(n,1);
  ntest = LIBXS_MAX(ntest,1);
#ifdef TEST_SINGLE
  nmat = LIBXS_MAX(VLENS,nmat - (nmat%VLENS));
#else
  nmat = LIBXS_MAX(VLEND,nmat - (nmat%VLEND));
#endif
  layout = LIBXS_MAX(LIBXS_MIN(layout,102),101);

  if ( layout == 102 ) lda = LIBXS_MAX(lda,m);
  else                 lda = LIBXS_MAX(lda,n);
  ...
  desc8 = libxs_getrf_descriptor_init(&blob, typesize8, m, n, lda, layout);
#ifdef TEST_SINGLE
  desc4 = libxs_getrf_descriptor_init(&blob, typesize4, m, n, lda, layout);
#endif
#ifdef USE_XSMM_GENERATED
  printf("calling libxs_dispatch_getrf: typesize8=%u\n",typesize8);
  mykernel = libxs_dispatch_getrf(desc8);
  printf("done calling libxs_dispatch_getrf: typesize8=%u\n",typesize8);
  if ( mykernel == NULL ) printf("R8 Kernel after the create call is null\n");
#ifdef TEST_SINGLE
  mykernel = libxs_dispatch_getrf(desc4);
  ...
  large_entry = LIBXS_MIN(256,sizea);
  large_entry = large_entry - (large_entry%16);
  while ( large_entry > m*n*nmat ) {
     large_entry /= 2;
  }
  large_entry = LIBXS_MAX(large_entry,4);
...
# include <mkl.h>
  ...
  for ( i = 0 , num = 0; i < (int)nmat; i+= (int)VLEND, num++ )
  {
     ...
     dtmp = libxs_timer_duration(l_start,l_end);
#endif
...
#else
  l_end = libxs_timer_tick();
  timer2 += libxs_timer_duration(l_start,l_end);
#endif
