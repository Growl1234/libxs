...
int main(void)
{
  ...
  result = libxs_matdiff(&da, LIBXS_DATATYPE(ITYPE), 3/*m*/, 3/*n*/,
    a/*ref*/, b/*tst*/, NULL/*ldref*/, NULL/*ldtst*/);

  if (EXIT_SUCCESS == result) {
    const double epsilon = libxs_matdiff_epsilon(&da);
    libxs_matdiff_reduce(&diff, &da);
    /* Epsilon (combined) */
    if (0.0000001 < LIBXS_ABS(epsilon - 0.1132714)) result = EXIT_FAILURE;
    /* One-norm */
    if (0.0000003 < LIBXS_ABS(da.norm1_abs - 1.8300000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(da.norm1_rel - 0.0963158)) result = EXIT_FAILURE;
    /* Infinity-norm */
    if (0.0000002 < LIBXS_ABS(da.normi_abs - 2.4400000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(da.normi_rel - 0.0976000)) result = EXIT_FAILURE;
    /* Froebenius-norm (relative) */
    if (0.0000001 < LIBXS_ABS(da.normf_rel - 0.1074954)) result = EXIT_FAILURE;
    /* L2-norm */
    if (0.0000002 < LIBXS_ABS(da.l2_abs - 1.8742465)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(da.l2_rel - 0.6726295)) result = EXIT_FAILURE;
    /** L1-norm */
    if (0.0000001 < LIBXS_ABS(da.l1_ref - 46.00)) result = EXIT_FAILURE;
    if (0.0000007 < LIBXS_ABS(da.l1_tst - 45.66)) result = EXIT_FAILURE;
    /* Linf-norm */
    if (0.0000004 < LIBXS_ABS(da.linf_abs - 0.9300000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(da.linf_rel - 0.5600000)) result = EXIT_FAILURE;
    /* R-squared */
    if (0.0000001 < LIBXS_ABS(da.rsq - 0.9490077)) result = EXIT_FAILURE;
  ...
  if (EXIT_SUCCESS == result) {
    result = libxs_matdiff(&db, LIBXS_DATATYPE(ITYPE), 1/*m*/, 3/*n*/,
      x/*ref*/, y/*tst*/, NULL/*ldref*/, NULL/*ldtst*/);
  }
  if (EXIT_SUCCESS == result) {
    const double epsilon = libxs_matdiff_epsilon(&db);
    libxs_matdiff_reduce(&diff, &db);
    /* Epsilon (combined) */
    if (0.0000001 < LIBXS_ABS(epsilon - 0.0223103)) result = EXIT_FAILURE;
    /* One-norm */
    if (0.0000001 < LIBXS_ABS(db.norm1_abs - 3.1000000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(db.norm1_rel - 0.0281818)) result = EXIT_FAILURE;
    /* Infinity-norm */
    if (0.0000001 < LIBXS_ABS(db.normi_abs - 2.0000000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(db.normi_rel - 0.0200000)) result = EXIT_FAILURE;
    /* Froebenius-norm (relative) */
    if (0.0000001 < LIBXS_ABS(db.normf_rel - 0.0222918)) result = EXIT_FAILURE;
    /** L2-norm */
    if (0.0000001 < LIBXS_ABS(db.l2_abs - 2.2383029)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(db.l2_rel - 0.2438908)) result = EXIT_FAILURE;
    /** L1-norm */
    if (0.0000001 < LIBXS_ABS(db.l1_ref - 110.00)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(db.l1_tst - 111.10)) result = EXIT_FAILURE;
    /* Linf-norm */
    if (0.0000001 < LIBXS_ABS(db.linf_abs - 2.0000000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(db.linf_rel - 0.2222222)) result = EXIT_FAILURE;
    /* R-squared */
    if (0.0000001 < LIBXS_ABS(db.rsq - 0.9991717)) result = EXIT_FAILURE;
    /* Location of maximum absolute error */
  ...
  if (EXIT_SUCCESS == result) {
    result = libxs_matdiff(&dc, LIBXS_DATATYPE(ITYPE), 3/*m*/, 1/*n*/,
      x/*ref*/, y/*tst*/, NULL/*ldref*/, NULL/*ldtst*/);
  }
  if (EXIT_SUCCESS == result) {
    const double epsilon = libxs_matdiff_epsilon(&dc);
    libxs_matdiff_reduce(&diff, &dc);
    /* Epsilon (combined) */
    if (0.0000001 < LIBXS_ABS(epsilon - 0.0223103)) result = EXIT_FAILURE;
    /* One-norm */
    if (0.0000001 < LIBXS_ABS(dc.norm1_abs - 3.1000000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(dc.norm1_rel - 0.0281818)) result = EXIT_FAILURE;
    /* Infinity-norm */
    if (0.0000001 < LIBXS_ABS(dc.normi_abs - 2.0000000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(dc.normi_rel - 0.0200000)) result = EXIT_FAILURE;
    /* Froebenius-norm (relative) */
    if (0.0000001 < LIBXS_ABS(dc.normf_rel - 0.0222918)) result = EXIT_FAILURE;
    /** L2-norm */
    if (0.0000001 < LIBXS_ABS(dc.l2_abs - 2.2383029)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(dc.l2_rel - 0.2438908)) result = EXIT_FAILURE;
    /** L1-norm */
    if (0.0000001 < LIBXS_ABS(dc.l1_ref - 110.00)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(dc.l1_tst - 111.10)) result = EXIT_FAILURE;
    /* Linf-norm */
    if (0.0000001 < LIBXS_ABS(dc.linf_abs - 2.0000000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(dc.linf_rel - 0.2222222)) result = EXIT_FAILURE;
    /* R-squared */
    if (0.0000001 < LIBXS_ABS(dc.rsq - 0.9991717)) result = EXIT_FAILURE;
    /* Location of maximum absolute error */
  ...
  if (EXIT_SUCCESS == result) {
    result = libxs_matdiff(&dd, LIBXS_DATATYPE(ITYPE), 3/*m*/, 1/*n*/,
      r/*ref*/, t/*tst*/, NULL/*ldref*/, NULL/*ldtst*/);
  }
  if (EXIT_SUCCESS == result) {
    const double epsilon = libxs_matdiff_epsilon(&dd);
    libxs_matdiff_reduce(&diff, &dd);
    /* Epsilon (combined) */
    if (0.0000001 < LIBXS_ABS(epsilon - 0.0006004)) result = EXIT_FAILURE;
    /* One-norm */
    if (0.0000001 < LIBXS_ABS(dd.norm1_abs - 0.0400000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(dd.norm1_rel - 0.0400000)) result = EXIT_FAILURE;
    /* Infinity-norm */
    ...
    /** L1-norm */
    if (0.0000001 < LIBXS_ABS(dd.l1_ref - 0.00)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(dd.l1_tst - 0.04)) result = EXIT_FAILURE;
    /* Linf-norm */
  ...
  if (EXIT_SUCCESS == result) {
    const double epsilon = libxs_matdiff_epsilon(&de);
    /* intentionally not considered: libxs_matdiff_reduce(&diff, &de) */
    /* Epsilon (combined) */
    if (0.0000001 < LIBXS_ABS(epsilon - 1.0000000)) result = EXIT_FAILURE;
    /* One-norm */
  ...
  if (EXIT_SUCCESS == result) {
    const double epsilon = libxs_matdiff_epsilon(&df);
