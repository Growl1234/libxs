...
int main(void)
{
  ...
  result = libxs_matdiff(&diff, LIBXS_DATATYPE(ITYPE), 3/*m*/, 3/*n*/,
    a/*ref*/, b/*tst*/, NULL/*ldref*/, NULL/*ldtst*/);

  if (EXIT_SUCCESS == result) {
    /* One-norm */
    if (0.0000003 < LIBXS_ABS(diff.norm1_abs - 1.8300000)) result = EXIT_FAILURE;
    ...
    /* L2-norm */
    if (0.0000002 < LIBXS_ABS(diff.l2_abs - 1.8742465)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(diff.l2_rel - 0.6726295)) result = EXIT_FAILURE;
    /** L1-norm */
    if (0.0000001 < LIBXS_ABS(diff.l1_ref - 46.00)) result = EXIT_FAILURE;
    if (0.0000007 < LIBXS_ABS(diff.l1_tst - 45.66)) result = EXIT_FAILURE;
    /* Linf-norm */
    ...
    if (a[3*diff.n+diff.m] != diff.v_ref) result = EXIT_FAILURE;
    if (b[3*diff.n+diff.m] != diff.v_tst) result = EXIT_FAILURE;
  }
  ...
  if (EXIT_SUCCESS == result) {
    ...
    /* Infinity-norm */
    if (0.0000001 < LIBXS_ABS(diff.normi_abs - 2.0000000)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(diff.normi_rel - 0.0200000)) result = EXIT_FAILURE;
    /* Froebenius-norm (relative) */
    if (0.0000001 < LIBXS_ABS(diff.normf_rel - 0.0222918)) result = EXIT_FAILURE;
    /** L2-norm */
    ...
    /** L1-norm */
    if (0.0000001 < LIBXS_ABS(diff.l1_ref - 110.0)) result = EXIT_FAILURE;
    if (0.0000001 < LIBXS_ABS(diff.l1_tst - 111.1)) result = EXIT_FAILURE;
    /* Linf-norm */
    ...
    if (x[3*diff.n+diff.m] != diff.v_ref) result = EXIT_FAILURE;
    if (y[3*diff.n+diff.m] != diff.v_tst) result = EXIT_FAILURE;
  }
