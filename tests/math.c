...
int main(int argc, char* argv[])
{
  ...
  { /* check coprime/2 routine */
    ...
    const int n = sizeof(test) / sizeof(*test);
    for (i = 0; i < n; ++i) {
      const size_t coprime = libxs_coprime2(test[i]);
      const unsigned int gcd = (unsigned int)libxs_gcd(coprime, test[i]);
      if ((0 != coprime || 1 < test[i]) && (test[i] <= coprime || 1 != gcd)) {
        exit(EXIT_FAILURE);
      }
    }
    if (libxs_coprime2(65423) != 32711) exit(EXIT_FAILURE);
    if (libxs_coprime2(1030) != 513) exit(EXIT_FAILURE);
    if (libxs_coprime2(1029) != 514) exit(EXIT_FAILURE);
    if (libxs_coprime2(1028) != 513) exit(EXIT_FAILURE);
    if (libxs_coprime2(1027) != 513) exit(EXIT_FAILURE);
    if (libxs_coprime2(1026) != 511) exit(EXIT_FAILURE);
    if (libxs_coprime2(1025) != 512) exit(EXIT_FAILURE);
    if (libxs_coprime2(1024) != 511) exit(EXIT_FAILURE);
    if (libxs_coprime2(1000) != 499) exit(EXIT_FAILURE);
    if (libxs_coprime2(997) != 498) exit(EXIT_FAILURE);
    if (libxs_coprime2(24) != 11) exit(EXIT_FAILURE);
    if (libxs_coprime2(5) != 2) exit(EXIT_FAILURE);
  }
