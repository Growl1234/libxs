...
# else
    void (*const handler)(int) = signal(SIGILL, internal_libxs_cpuid_arm_sigill);
    ...
    if (SIG_ERR != handler) {
        ...
        || /* DPB */ 0 != (0xF & id_aa64isar1_el1))
      {
        ...
        volatile int no_access = 0; /* try internal_libxs_cpuid_arm_svcntb */
        ...
        if (0 != (0xF & (id_aa64pfr0_el1 >> 32)) || 0 != no_access) { /* SVE */
          const int vlen_bytes = internal_libxs_cpuid_arm_svcntb();
          switch (vlen_bytes) {
            case 16: { /* SVE 128-bit */
...
# if !defined(NDEBUG)
  ...
  {
    fprintf(stderr, "LIBXS WARNING: internal_libxs_cpuid_arm called on non-ARM platform!\n");
  }
# endif
