...
LIBXS_API_INTERN libxs_dnn_err_t libxs_dnn_softmaxloss_st_bwd_ncnc_f32_f32(libxs_dnn_softmaxloss* handle, int start_thread, int tid);
LIBXS_API_INTERN libxs_dnn_err_t libxs_dnn_softmaxloss_st_bwd_ncnc_bf16_bf16(libxs_dnn_softmaxloss* handle, int start_thread, int tid);


LIBXS_API_INTERN LIBXS_INTRINSICS(LIBXS_X86_AVX512)
libxs_dnn_err_t libxs_dnn_softmaxloss_st_bwd_ncnc_f32_f32(libxs_dnn_softmaxloss* handle, int start_thread, int tid)
{
...
LIBXS_API_INTERN LIBXS_INTRINSICS(LIBXS_X86_AVX512)
libxs_dnn_err_t libxs_dnn_softmaxloss_st_bwd_ncnc_bf16_bf16(libxs_dnn_softmaxloss* handle, int start_thread, int tid)
{
  libxs_dnn_err_t status = LIBXS_DNN_SUCCESS;
#if defined(LIBXS_INTRINSICS_AVX512) /*__AVX512F__*/
...
# define LIBXS_DNN_SOFTMAXLOSS_BWD_BF16_AVX512
# include "template/libxs_dnn_softmaxloss_st_bwd_ncnc_generic.tpl.c"
# undef LIBXS_DNN_SOFTMAXLOSS_BWD_BF16_AVX512
#else /* should not happen */
  LIBXS_UNUSED(handle); LIBXS_UNUSED(start_thread); LIBXS_UNUSED(tid);
  status = LIBXS_DNN_ERR_UNSUPPORTED_ARCH;
#endif
...
LIBXS_API_INTERN libxs_dnn_err_t libxs_dnn_softmaxloss_st_bwd_ncnc(libxs_dnn_softmaxloss* handle, int start_thread, int tid)
{
  ...
  if ( libxs_target_archid >= LIBXS_X86_AVX512 ) {
    if ( handle->desc.datatype == LIBXS_DNN_DATATYPE_F32 ) {
      status = libxs_dnn_softmaxloss_st_bwd_ncnc_f32_f32( handle, start_thread, tid);
    } else if ( handle->desc.datatype == LIBXS_DNN_DATATYPE_BF16 ) {
      status = libxs_dnn_softmaxloss_st_bwd_ncnc_bf16_bf16( handle, start_thread, tid);
    } else {
