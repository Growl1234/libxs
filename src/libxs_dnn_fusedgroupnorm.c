...
LIBXS_API libxs_dnn_err_t libxs_dnn_destroy_fusedgroupnorm(const libxs_dnn_fusedgroupnorm* handle) {
  ...
  if (0 != handle) {
    ...
    free(/*remove constness*/(libxs_dnn_fusedgroupnorm*)handle);
  } else {
  ...
  *status = LIBXS_DNN_SUCCESS;
  ...
  if (handle != 0) {
    ...
    if (layout != 0) {
      ...
      } else if ( (type == LIBXS_DNN_CHANNEL_EXPECTVAL) || (type == LIBXS_DNN_CHANNEL_RCPSTDDEV) || (type == LIBXS_DNN_CHANNEL_VARIANCE) ) {
        layout->tensor_type = LIBXS_DNN_CHANNEL_SCALAR;

        if ( ((handle->desc.buffer_format & LIBXS_DNN_TENSOR_FORMAT_LIBXS) > 0) || ((handle->desc.buffer_format & LIBXS_DNN_TENSOR_FORMAT_NHWC) > 0) ) {
          if ( handle->desc.datatype_stats == LIBXS_DNN_DATATYPE_F32 ) {
            layout->datatype = handle->desc.datatype_stats;
            layout->dim_type = (libxs_dnn_tensor_dimtype*) malloc(2*sizeof(libxs_dnn_tensor_dimtype));
            layout->dim_size = (unsigned int*) malloc(2*sizeof(unsigned int));
            ...
            if (0 != layout->dim_type && 0 != layout->dim_size) {
              layout->num_dims = 2;
              layout->dim_type[0] = LIBXS_DNN_TENSOR_DIMTYPE_G;
...
LIBXS_API libxs_dnn_err_t libxs_dnn_fusedgroupnorm_bind_tensor(libxs_dnn_fusedgroupnorm* handle, const libxs_dnn_tensor* tensor, const libxs_dnn_tensor_type type) {
  ...
  if (handle != 0 && tensor != 0) {
    libxs_dnn_tensor_datalayout* handle_layout = libxs_dnn_fusedgroupnorm_create_tensor_datalayout(handle, type, &status);
  ...
  if (0 != handle) {
    switch (kind) {
      case LIBXS_DNN_COMPUTE_KIND_FWD: {
        switch (handle->desc.buffer_format) {
          case LIBXS_DNN_TENSOR_FORMAT_LIBXS: {
            status = libxs_dnn_fusedgroupnorm_st_fwd_custom( handle, start_thread, tid );
          } break;
          default: {
      ...
      case LIBXS_DNN_COMPUTE_KIND_BWD: {
        switch (handle->desc.buffer_format) {
          case LIBXS_DNN_TENSOR_FORMAT_LIBXS: {
            status = libxs_dnn_fusedgroupnorm_st_bwd_custom( handle, start_thread, tid );
          } break;
          default: {
            status = LIBXS_DNN_ERR_INVALID_FORMAT_FUSEDBN;
          }
