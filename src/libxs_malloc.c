...
# define LIBXS_MALLOC_LOCK_PAGES 1
     ...
     defined(LIBXS_MALLOC_MOD) && 0
# define LIBXS_MALLOC_LOCK_ALL
#endif
/* record real allocation size */
#if !defined(LIBXS_MALLOC_INFO_ALLOCSIZE) && 0
# define LIBXS_MALLOC_INFO_ALLOCSIZE
#endif
...
# define INTERNAL_MEMALIGN_HOOK(RESULT, FLAGS, ALIGNMENT, SIZE, CALLER) do { \
    const int internal_memalign_hook_recursive_ = LIBXS_ATOMIC_ADD_FETCH( \
      &internal_malloc_recursive, 1, LIBXS_ATOMIC_SEQ_CST); \
    ...
    LIBXS_ATOMIC_SUB_FETCH(&internal_malloc_recursive, 1, LIBXS_ATOMIC_SEQ_CST); \
  } while(0)
# define INTERNAL_REALLOC_HOOK(RESULT, FLAGS, PTR, SIZE, CALLER) \
  if (0 == (internal_malloc_kind & 1) || 0 >= internal_malloc_kind \
    /*|| (0 != LIBXS_ATOMIC_LOAD(&internal_malloc_recursive, LIBXS_ATOMIC_SEQ_CST))*/ \
    || (internal_malloc_limit[0] > (SIZE)) \
    || (internal_malloc_limit[1] < (SIZE) && 0 != internal_malloc_limit[1])) \
  { \
...
internal_malloc_info_type* internal_malloc_info(const void* memory, int check)
{
  ...
  LIBXS_VALUE_ASSIGN(result, buffer_info);
  ...
  if (0 != check && NULL != result) { /* check ownership */
...
#endif
    {
      ...
      union { libxs_free_fun fun; const void* ptr; } convert = { 0 };
...
# if (!defined(NDEBUG) || defined(LIBXS_MALLOC_SCRATCH_TRIM_HEAD))
  ...
  LIBXS_VALUE_ASSIGN(buffer, memory);
...
# else
      internal_xfree(pool_buffer, info); /* !libxs_free, invalidates info */
# endif
    }
  }
...
# endif
    ...
    { /* local memory allocation */
        ...
        && 1 == LIBXS_ATOMIC_ADD_FETCH(&error_once, 1, LIBXS_ATOMIC_RELAXED))
      {
        ...
        assert(NULL == *memory); /* !LIBXS_ASSERT */
      }
...
#if (defined(LIBXS_MALLOC_HOOK) && defined(LIBXS_MALLOC) && (0 != LIBXS_MALLOC)) || defined(LIBXS_MALLOC_MOD)
...
# endif
          | (0 != (LIBXS_MALLOC_FLAG_X & flags) ? LIBXS_MAP_JIT : 0)
        ; /* mflags */
# if defined(MAP_POPULATE)
        ...
        NULL != buffer)
      {
        ...
        if (NULL != *memory) { /* preserve/copy previous content */
          ...
          if (NULL != info /* known allocation (non-foreign pointer) */
            && EXIT_SUCCESS != internal_xfree(*memory, info) /* !libxs_free, invalidates info */
            && 0 != libxs_verbosity /* library code is expected to be mute */
            && 1 == LIBXS_ATOMIC_ADD_FETCH(&error_once, 1, LIBXS_ATOMIC_RELAXED))
...
# else
      internal_xfree(memory, info); /* !libxs_free, invalidates info */
# endif
    }
    else {
# if defined(LIBXS_MALLOC_SCRATCH_MAX_NPOOLS) && (0 < (LIBXS_MALLOC_SCRATCH_MAX_NPOOLS))
