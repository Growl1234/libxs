...
LIBXS_EXTERN int posix_memalign(void**, size_t, size_t) LIBXS_NOEXCEPT;
# else
...
# endif
        {
          ...
          if ( kernabi == 0 ) {
            ...
            LIBXS_SNPRINTF(jit_name, sizeof(jit_name), "libxs_abi%i_%s_%s_%c%c_%ux%ux%u_%u_%u_%u_a%i_b%i_p%i_br%i_uh%u_si%i_tc-%s_avnni%i_bvnni%i_cvnni%i.mxm", kernabi, target_arch, tname,
