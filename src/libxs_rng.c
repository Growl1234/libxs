...
{
  internal_libxs_rng_state = (unsigned long long)seed;
}
    ...
    if (leftover < n) { /* rejection branch (rare for small n) */
      ...
      while (leftover < threshold) {
        m = (unsigned long long)(unsigned int)
          internal_libxs_rng_splitmix64(&internal_libxs_rng_state) * n;
...
LIBXS_API void libxs_rng_seq(void* data, size_t nbytes)
{
  ...
  for (; dst < end; dst += 8) {
    unsigned long long r = internal_libxs_rng_splitmix64(&internal_libxs_rng_state);
