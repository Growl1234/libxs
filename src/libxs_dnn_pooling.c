  ...
  if ( ((pooling_desc.datatype_in == LIBXS_DNN_DATATYPE_BF16) && (pooling_desc.datatype_out == LIBXS_DNN_DATATYPE_BF16)) ||
    ...
    if (0 != handle) {
      ...
      *status = libxs_dnn_get_feature_map_blocks( handle->desc.C, handle->desc.C,
      ...
      handle->scratch_size = (sizeof(float) * ( (size_t)handle->desc.H + (size_t)LIBXS_MAX(handle->desc.pad_h_in, handle->desc.pad_h_out)*2 )
                                            * ( (size_t)handle->desc.W + (size_t)LIBXS_MAX(handle->desc.pad_w_in, handle->desc.pad_w_out)*2 )
...
LIBXS_API libxs_dnn_err_t libxs_dnn_destroy_pooling(const libxs_dnn_pooling* handle) {
  ...
  if (0 != handle) {
    ...
    free(/*remove constness*/(libxs_dnn_pooling*)handle);
  } else {
      ...
      if ( (type == LIBXS_DNN_REGULAR_INPUT)     || (type == LIBXS_DNN_GRADIENT_INPUT)  || (type == LIBXS_DNN_INPUT)  ||
        ...
        if ((handle->desc.buffer_format & LIBXS_DNN_TENSOR_FORMAT_LIBXS) > 0) {
          if ( ((handle->desc.datatype_in == LIBXS_DNN_DATATYPE_F32) && (handle->desc.datatype_out == LIBXS_DNN_DATATYPE_F32) ) ) {
            if ( type == LIBXS_DNN_POOLING_MASK ) {
...
LIBXS_API libxs_dnn_err_t libxs_dnn_pooling_bind_tensor(libxs_dnn_pooling* handle, const libxs_dnn_tensor* tensor, const libxs_dnn_tensor_type type) {
  ...
  if (handle != 0 && tensor != 0) {
    libxs_dnn_tensor_datalayout* handle_layout = libxs_dnn_pooling_create_tensor_datalayout(handle, type, &status);
    ...
    if ( libxs_dnn_compare_tensor_datalayout(handle_layout, tensor->layout, &status) == 0 ) {
      if ( type == LIBXS_DNN_REGULAR_INPUT ) {
        ...
        handle->mask = (libxs_dnn_tensor*)tensor;
      } else {
        /* cannot happen */
      }
    } else {
  ...
  if (0 != handle) {
    switch (kind) {
      case LIBXS_DNN_COMPUTE_KIND_FWD: {
        switch (handle->desc.buffer_format) {
          case LIBXS_DNN_TENSOR_FORMAT_LIBXS: {
            status = libxs_dnn_pooling_st_fwd_custom( handle, start_thread, tid );
          } break;
          default: {
      ...
      case LIBXS_DNN_COMPUTE_KIND_BWD: {
        switch (handle->desc.buffer_format) {
          case LIBXS_DNN_TENSOR_FORMAT_LIBXS: {
            status = libxs_dnn_pooling_st_bwd_custom( handle, start_thread, tid );
          } break;
          default: {
            status = LIBXS_DNN_ERR_INVALID_FORMAT_FUSEDBN;
          }
