  ...
  const unsigned char* word, int len)
{
  ...
  if (NULL != word && 0 < len) {
    ...
    result = 5381;
    for (i = 0; i < len; ++i) {
      result = ((result << 5) + result) ^ (unsigned int)word[i];
    }
  }
  return result;
}
...
{
  int result = 0;
  if (NULL != text && 0 < byte_pos) {
    int wstart = byte_pos - 1, wlen;
    while (wstart > 0 && 0 == isspace(text[wstart - 1])
      && 0 == ispunct(text[wstart - 1]))
    {
      --wstart;
    }
  ...
  const libxs_textrule_ctx_t* ctx)
{
  ...
  switch ((int)rule->tmpl) {
    ...
    case LIBXS_TRULE_NEXT_CHAR_UPPER: {
      ...
      while (npos < (int)ctx->text_size && 0 != isspace(ctx->text[npos])) {
        ++npos;
      }
      if (npos < (int)ctx->text_size && 0 != isupper(ctx->text[npos])) {
        result = 1;
      }
    } break;
    case LIBXS_TRULE_NEXT_CHAR_LOWER: {
      ...
      if (npos < (int)ctx->text_size && 0 != islower(ctx->text[npos])) {
        result = 1;
      }
    } break;
    case LIBXS_TRULE_PREV_TOKEN_BREAK: {
      if (NULL != ctx->prev_token && 0 != libxs_lexeme_has_break(ctx->prev_token)) {
        result = 1;
      }
    } break;
...
{
  int result = 0, i;
  if (NULL != ctx && NULL != ctx->token
    && 0 != libxs_lexeme_is_sentence_end(ctx->token))
  {
    result = 1;
    if (NULL != rules) {
      for (i = 0; i < nrules; ++i) {
  ...
  libxs_textrule_t* rules, int max_rules)
{
  ...
  if (NULL != registry && NULL != rules && 0 < max_rules) {
    memcpy(key, "TRULE:", 6);
...
LIBXS_API int libxs_textrule_save(libxs_registry_t* registry,
  const libxs_textrule_t* rules, int nrules)
{
  int result = (NULL != registry && NULL != rules)
  ...
  const libxs_lexrule_t* rules, int nrules)
{
  ...
  if (EXIT_SUCCESS == result) {
    memcpy(key, "LRULE:", 6);
    for (rule_pos = 0; rule_pos < nrules && EXIT_SUCCESS == result;
      ++rule_pos)
    {
      size_t key_size;
  ...
  size_t pos)
{
  ...
  if (i < size) {
    if (0 != isdigit(text[i])) {
      ...
      if (j < size && ('.' == text[j] || ')' == text[j])) result = 1;
    }
    if (0 == result && ('-' == text[i] || '*' == text[i] || '+' == text[i])) {
      if (i + 1 < size && ' ' == text[i + 1]) result = 1;
    }
    if (0 == result && '(' == text[i] && i + 1 < size
      && 0 != isalpha(text[i + 1]) && i + 2 < size && ')' == text[i + 2])
    {
      result = 1;
    }
  }
...
int internal_reflow_is_structural(const unsigned char* text, size_t size,
  size_t nl_pos)
{
  /* structural unless one of the tests below says the break is a reflow */
  int result = 1;
  if (nl_pos + 1 < size
    && '\n' != text[nl_pos + 1] && '\r' != text[nl_pos + 1])
  {
    size_t line_start = nl_pos, next_start, prev_len;
    while (line_start > 0 && '\n' != text[line_start - 1]) --line_start;
    prev_len = nl_pos - line_start;
    next_start = nl_pos + 1;
    if (next_start < size && '\r' == text[next_start]) ++next_start;
    if (0 != prev_len
      && 0 == internal_reflow_line_start(text, size, next_start))
    {
      int next_indent = 0, cur_indent = 0;
      { size_t k = next_start;
      ...
      if (next_indent < 4 || next_indent <= cur_indent + 2) {
        int caption = 0;
        if (prev_len < 40) {
          ...
          for (k = line_start; k < nl_pos; ++k) {
            ...
            if (k == line_start
              || (0 != isspace(text[k - 1]) && 0 == isspace(text[k])))
            {
              ++nwords;
            }
          }
          caption = (0 != all_upper && nwords <= 6)
            || (nwords <= 3 && prev_len < 25);
        }
        if (0 == caption) {
          size_t k = next_start;
          while (k < size && ' ' == text[k]) ++k;
          if (k < size && 0 != islower(text[k]) && prev_len >= 40) result = 0;
          else if (60 <= prev_len) result = 0;
        }
      }
    }
  }
  ...
  unsigned char** out, size_t* out_size, size_t** line_offsets, size_t* nlines)
{
  ...
  size_t nline = 0;
  size_t i, j;
  if (NULL != text && NULL != out && NULL != out_size) {
    buf = (unsigned char*)malloc(size + 1);
    if (NULL != buf && NULL != line_offsets && NULL != nlines) {
