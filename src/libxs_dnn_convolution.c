...
LIBXS_API_INLINE libxs_dnn_err_t libxs_dnn_convolution_setup( libxs_dnn_layer* handle ) {
  ...
  if (handle->datatype_in == LIBXS_DNN_DATATYPE_BF16) {
    ...
    handle->fwd_cvtfp32bf16_kernel = libxs_dispatch_meltw_cvtfp32bf16(handle->ofmblock * handle->fwd_ofw_rb, handle->fwd_ofh_rb, &_ldi, &_ldo, LIBXS_DATATYPE_F32, LIBXS_DATATYPE_BF16, 0);
  }
