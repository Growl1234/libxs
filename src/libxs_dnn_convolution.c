...
/**********************************************************/
LIBXS_API_INLINE int libxs_dnn_convolution_setup_ifmblock( libxs_dnn_layer* handle ) {
  int result = 1;
  int ofm, lp;

  libxs_dnn_get_feature_map_blocks( handle->desc.C, handle->desc.K, &result, &ofm, &lp, handle->desc.datatype_in, handle->desc.datatype_out );

  return result;
}

LIBXS_API_INLINE int libxs_dnn_convolution_setup_ofmblock( libxs_dnn_layer* handle ) {
  int result = 1;
  int ifm, lp;

  libxs_dnn_get_feature_map_blocks( handle->desc.C, handle->desc.K, &ifm, &result, &lp, handle->desc.datatype_in, handle->desc.datatype_out );

  return result;
}

LIBXS_API_INLINE int libxs_dnn_convolution_setup_fm_lp_block( libxs_dnn_layer* handle ) {
  int result = 1;
  int ifm, ofm;

  libxs_dnn_get_feature_map_blocks( handle->desc.C, handle->desc.K, &ifm, &ofm, &result, handle->desc.datatype_in, handle->desc.datatype_out );

  return result;
}

LIBXS_API_INLINE int libxs_dnn_convolution_setup_fallback_loops_fwd( libxs_dnn_layer* handle ) {
  int result = 0;
...
LIBXS_API_INLINE int libxs_dnn_convolution_setup_blocksifm( libxs_dnn_layer* handle ) {
  int result = handle->desc.C / handle->ifmblock;
  return result;
}

LIBXS_API_INLINE int libxs_dnn_convolution_setup_blocksofm( libxs_dnn_layer* handle ) {
...
LIBXS_API_INLINE int libxs_dnn_convolution_setup_block_fwd_IFM( libxs_dnn_layer* handle ) {
  ...
  result = LIBXS_MIN(handle->blocksifm, result);
  return result;
}
...
LIBXS_API_INLINE void libxs_dnn_convolution_setup_fwd_scratch( libxs_dnn_layer* handle ) {
  ...
  if ( ( handle->datatype_in == LIBXS_DNN_DATATYPE_BF16 ) ||
    ...
    handle->fwd_lp_output_full_scratch_size = (size_t) LIBXS_MAX(handle->desc.threads * handle->fwd_gemm_pixels * handle->ofmblock * libxs_dnn_typesize(LIBXS_DNN_DATATYPE_F32), handle->desc.N * handle->desc.K * handle->ofwp * handle->ofhp * libxs_dnn_typesize(LIBXS_DNN_DATATYPE_F32));
...
LIBXS_API_INLINE void libxs_dnn_convolution_setup_bwd_scratch( libxs_dnn_layer* handle ) {
  ...
  if ( handle->datatype_in == LIBXS_DNN_DATATYPE_BF16 ) {
    handle->bwd_lp_input_full_scratch_size = (size_t) LIBXS_MAX(handle->desc.threads * handle->bwd_gemm_pixels * handle->ifmblock * libxs_dnn_typesize(LIBXS_DNN_DATATYPE_F32), handle->desc.N * handle->desc.C * handle->ifwp * handle->ifhp * libxs_dnn_typesize(LIBXS_DNN_DATATYPE_F32));
    /* logical padding with copying in the fly */
    if ( handle->use_fallback_bwd_loops != 0 ) {
      handle->bwd_packing_padding_scratch_size = (size_t)handle->desc.threads * handle->ifmblock *
        (handle->desc.H + 2*handle->desc.pad_h) *
...
LIBXS_API_INLINE void libxs_dnn_convolution_setup_bf16_upd( libxs_dnn_layer* handle ) {
  ...
  if (handle->upd_linearized_pixels == 1) {
    ...
    if (handle->ofw <= 14) {
      ...
      handle->weight_copies = libxs_dnn_convolution_setup_weight_copies_upd(handle);
      if (handle->ofw == 14 && handle->desc.K >= 1024) {
        handle->use_hybrid_imgofm_parallelization = 0;
  ...
  if (handle->desc.N != handle->desc.threads) {
    ...
    handle->use_hybrid_imgofm_parallelization = 0;
    handle->weight_copies = LIBXS_MIN(handle->desc.N, handle->desc.threads);
  }
...
LIBXS_API_INLINE void libxs_dnn_convolution_setup_bf16_upd_amx( libxs_dnn_layer* handle ) {
  ...
  handle->use_hybrid_imgofm_parallelization = 0;
  handle->upd_linearized_tasklist = 0;
  if (((handle->target_archid == LIBXS_X86_AVX512_SPR) && (handle->target_archid <= LIBXS_X86_ALLFEAT)) && (handle->ofw == 7) && (handle->desc.R == 1) && (handle->desc.S == 1) ) {
  ...
  if (handle->upd_linearized_pixels == 0) {
    ...
    stride_a = handle->ofwp_extended * handle->ofmblock * libxs_dnn_typesize(handle->datatype_in);
    stride_b = handle->desc.u * handle->ifwp_extended * libxs_dnn_typesize(handle->datatype_in);
    handle->upd_config_kernel = libxs_bsmmdispatch(handle->ofmblock, handle->ifmblock, handle->ofw+handle->remainder_pixels, &LDA, &LDB, &LDC, NULL, &beta, &l_tc_flags, NULL);
    handle->upd_compute_kernel_brgemm_no_linearized_pixels = libxs_bsmmdispatch_reducebatch_strd_unroll(handle->ofmblock, handle->ifmblock, handle->ofw+handle->remainder_pixels,
      (libxs_blasint)stride_a, (libxs_blasint)stride_b, unroll_hint, &LDA, &LDB, &LDC, NULL, &beta, &l_flags, &prefetch_mode);
  } else {
    LDA = handle->ofmblock;
    ...
    if (handle->use_hybrid_imgofm_parallelization == 0) {
      handle->upd_config_kernel = libxs_bsmmdispatch(handle->ofmblock, handle->ifmblock, handle->pixel_blocking, &LDA, &LDB, &LDC, NULL, &beta, &l_tc_flags, NULL);
      handle->upd_compute_kernel_gemm_linearized_pixels_no_hybrid_par = libxs_bsmmdispatch(handle->ofmblock, handle->ifmblock, handle->pixel_blocking, &LDA, &LDB, &LDC, NULL, &beta, &l_flags, &prefetch_mode);
    } else {
      if (handle->pack_to_cnhw == 1) {
        handle->upd_config_kernel = libxs_bsmmdispatch(handle->ofmblock, handle->ifmblock, handle->pixel_blocking, &LDA, &LDB, &LDC, NULL, &beta, &l_tc_flags, NULL);
        handle->upd_compute_kernel_gemm_linearized_pixels_hybrid_par_cnhw = libxs_bsmmdispatch(handle->ofmblock, handle->ifmblock, handle->pixel_blocking, &LDA, &LDB, &LDC, NULL, &beta, &l_flags, &prefetch_mode);
      } else {
        /* TODO: Hoist here hybrid parallelization logic and then we should be able to also provide unroll hint in the BRGEMM call */
        stride_a = handle->blocksofm * handle->output_pixels * handle->ofmblock * libxs_dnn_typesize(handle->datatype_in);
        stride_b = handle->blocksifm * handle->ifmblock * handle->input_pixels * libxs_dnn_typesize(handle->datatype_in);
        handle->upd_config_kernel = libxs_bsmmdispatch(handle->ofmblock, handle->ifmblock, handle->pixel_blocking, &LDA, &LDB, &LDC, NULL, &beta, &l_tc_flags, NULL);
        handle->upd_compute_kernel_brgemm_linearized_pixels_hybrid_par_no_cnhw = libxs_bsmmdispatch_reducebatch_strd(handle->ofmblock, handle->ifmblock, handle->pixel_blocking,
          (libxs_blasint)stride_a, (libxs_blasint)stride_b, &LDA, &LDB, &LDC, NULL, &beta, &l_flags, &prefetch_mode);
      }
    }
  }
...
LIBXS_API_INLINE void libxs_dnn_convolution_setup_upd_scratch( libxs_dnn_layer* handle ) {
  ...
  handle->upd_lp_filter_full_scratch_size += ( handle->upd_lp_filter_full_scratch_size % LIBXS_CACHELINE == 0 ) ? 0 :
    LIBXS_CACHELINE - (handle->upd_lp_filter_full_scratch_size % LIBXS_CACHELINE);
...
LIBXS_API_INLINE libxs_dnn_err_t libxs_dnn_convolution_setup( libxs_dnn_layer* handle ) {
  ...
  if ( ((handle->target_archid == LIBXS_X86_AVX512_SPR) && (handle->target_archid <= LIBXS_X86_ALLFEAT)) && (handle->datatype_in == LIBXS_DNN_DATATYPE_BF16) && ((handle->desc.C % 16 != 0) || (handle->desc.K % 16 != 0)) ) {
    handle->target_archid = LIBXS_X86_AVX512_CPX;
  }
  handle->ifmblock = libxs_dnn_convolution_setup_ifmblock(handle);
  handle->ofmblock = libxs_dnn_convolution_setup_ofmblock(handle);
  handle->fm_lp_block = libxs_dnn_convolution_setup_fm_lp_block(handle);
  handle->blocksifm = libxs_dnn_convolution_setup_blocksifm(handle);
  handle->blocksofm = libxs_dnn_convolution_setup_blocksofm(handle);

  /* If in SPR, generate tilerelease kernel */
  if (handle->target_archid >= LIBXS_X86_AVX512_SPR) {
    int l_tr_flags = LIBXS_GEMM_FLAG_NO_SETUP_TILECONFIG | ( LIBXS_GEMM_VNNI_FLAGS('N', 'N', 'V', 'N') );
    handle->tilerelease_kernel = libxs_bsmmdispatch(handle->ifmblock, handle->ifmblock, handle->ifmblock, NULL, NULL, NULL, NULL, NULL, &l_tr_flags, NULL);
  }

  /* FWD parameter setup  */
  handle->fwd_ofw_rb = libxs_dnn_convolution_setup_fwd_ofw_rb(handle);
  handle->pack_input = libxs_dnn_convolution_setup_pack_input_fwd(handle);
  handle->fwd_ofh_rb = libxs_dnn_convolution_setup_fwd_ofh_rb(handle);
  handle->fwd_gemm_pixels = libxs_dnn_convolution_setup_fwd_pixels_gemm(handle);
  handle->block_fwd_oj = libxs_dnn_convolution_setup_fwd_block_H(handle);
  ...
  if ( ((handle->target_archid == LIBXS_X86_AVX512_SPR) && (handle->target_archid <= LIBXS_X86_ALLFEAT)) && (handle->datatype_in == LIBXS_DNN_DATATYPE_BF16) ) {
    ...
    if (handle->desc.R == 1 && handle->desc.S == 1) {
      const int IFW = (handle->pack_input == 1) ? handle->ofwp : handle->ifwp;
      const int IFH = (handle->pack_input == 1) ? handle->ofhp : handle->ifhp;
      size_t stride_a = handle->desc.R * handle->desc.S * handle->ifmblock * handle->ofmblock * libxs_dnn_typesize(handle->datatype_in);
      size_t stride_b = IFW * IFH * handle->ifmblock * libxs_dnn_typesize(handle->datatype_in);
      handle->fwd_compute_kernel_strd = libxs_bmmdispatch_reducebatch_strd_unroll(handle->ofmblock, handle->fwd_gemm_pixels, handle->ifmblock,
        (libxs_blasint)stride_a, (libxs_blasint)stride_b, handle->blocksifm_blocking, &ldA, &ldx, &ldC, NULL, &beta, &l_flags, NULL);
    } else {
      const int IFW = (handle->fwd_padding_copy == 1) ? handle->ifwp + 2*handle->desc.pad_w : ( (handle->pack_input == 1) ? handle->ofwp : handle->ifwp );
      const int IFH = (handle->fwd_padding_copy == 1) ? handle->ifhp + 2*handle->desc.pad_h : ( (handle->pack_input == 1) ? handle->ofhp : handle->ifhp );
      int n_blocks = handle->desc.R * handle->desc.S * handle->blocksifm_blocking;
  ...
  if ((handle->datatype_in == LIBXS_DNN_DATATYPE_I8) && (handle->datatype_out == LIBXS_DNN_DATATYPE_I32)) {
    ...
    l_flags = ( LIBXS_GEMM_VNNI_FLAGS('N', 'N', 'V', 'N') ) | handle->fwd_flags;
    if (handle->desc.R == 1 && handle->desc.S == 1) {
      const int IFW = (handle->pack_input == 1) ? handle->ofwp : handle->ifwp;
      const int IFH = (handle->pack_input == 1) ? handle->ofhp : handle->ifhp;
      ...
      handle->gemm_fwd.xgemm.subimrs = libxs_subimmdispatch_reducebatch_strd(handle->ofmblock, handle->fwd_ofh_rb*handle->fwd_ofw_rb, handle->ifmblock, stride_A, stride_B, &ldA, &ldx, &ldC, NULL, &beta_int, &l_flags, NULL);
    } else {
      const int IFW = (handle->pack_input == 1) ? handle->ofwp : handle->ifwp;
      const int IFH = (handle->pack_input == 1) ? handle->ofhp : handle->ifhp;
      if (handle->avoid_fmas_in_rim == 0) {
        ...
        handle->gemm_fwd.xgemm.subimro = libxs_subimmdispatch_reducebatch_offs(handle->ofmblock, handle->fwd_ofh_rb*handle->fwd_ofw_rb, handle->ifmblock, &ldA, &ldx, &ldC, NULL, &beta_int, &l_flags, NULL);
      } else {
        libxs_blasint stride_A = handle->ifmblock * handle->desc.R * handle->desc.S * handle->ofmblock * sizeof(char);
        libxs_blasint stride_B = handle->ifmblock * IFW * IFH * sizeof(char) ;
        handle->gemm_fwd.xgemm.subimrs = libxs_subimmdispatch_reducebatch_strd(handle->ofmblock, handle->fwd_ofh_rb*handle->fwd_ofw_rb, handle->ifmblock, stride_A, stride_B, &ldA, &ldx, &ldC, NULL, &beta_int, &l_flags, NULL);
        handle->gemm_fwd2.xgemm.subimrs = libxs_subimmdispatch_reducebatch_strd(handle->ofmblock, handle->fwd_ofh_rb*(handle->fwd_ofw_rb-1), handle->ifmblock, stride_A, stride_B, &ldA, &ldx, &ldC, NULL, &beta_int, &l_flags, NULL);
      }
    }
  } else if ((handle->datatype_in == LIBXS_DNN_DATATYPE_I8) && (handle->datatype_out == LIBXS_DNN_DATATYPE_I8)) {
    ldx = (libxs_blasint)handle->desc.v*handle->ifmblock;
    ldA = handle->ofmblock;
    ldC = handle->ofmblock;
    beta_int = 0;
    l_flags = ( LIBXS_GEMM_VNNI_FLAGS('N', 'N', 'V', 'N') ) | handle->fwd_flags;
    if (handle->desc.R == 1 && handle->desc.S == 1) {
      ...
      libxs_blasint stride_A = handle->ifmblock * handle->ofmblock * sizeof(char);
      libxs_blasint stride_B = handle->ifmblock * IFW * IFH * sizeof(char) ;
      handle->gemm_fwd.xgemm.sububmrs = libxs_sububmmdispatch_reducebatch_strd(handle->ofmblock, handle->fwd_ofh_rb*handle->fwd_ofw_rb, handle->ifmblock, stride_A, stride_B, &ldA, &ldx, &ldC, NULL, &beta_int, &l_flags, NULL);
    } else {
      const int IFW = handle->ifwp;
      const int IFH = handle->ifhp;
      int n_blocks = handle->desc.R * handle->desc.S * handle->blocksifm_blocking;
      ...
      handle->gemm_fwd.xgemm.sububmro = libxs_sububmmdispatch_reducebatch_offs(handle->ofmblock, handle->fwd_ofh_rb*handle->fwd_ofw_rb, handle->ifmblock, &ldA, &ldx, &ldC, NULL, &beta_int, &l_flags, NULL);
    }
  }
  ...
  /* BWD parameter setup  */
  handle->bwd_ofw_rb = libxs_dnn_convolution_setup_bwd_ofw_rb(handle);
  handle->bwd_ofh_rb = libxs_dnn_convolution_setup_bwd_ofh_rb(handle);
  handle->bwd_gemm_pixels = libxs_dnn_convolution_setup_bwd_pixels_gemm(handle);
  handle->pack_input_bwd = libxs_dnn_convolution_setup_pack_input_bwd(handle);
  handle->spread_input_bwd = libxs_dnn_convolution_setup_spread_input_bwd(handle);
  handle->blocksofm_blocking = libxs_dnn_convolution_setup_blocksofm_blocking(handle);
  handle->avoid_acc_load_bwd = libxs_dnn_convolution_setup_avoid_acc_load_bwd(handle);
  handle->use_ifm_parallelization = libxs_dnn_convolution_setup_use_ifm_parallelization(handle);
  handle->block_bwd_ofm = libxs_dnn_convolution_setup_block_bwd_OFM(handle);
  handle->block_bwd_ifm = libxs_dnn_convolution_setup_block_bwd_IFM(handle);
  handle->block_bwd_oj = libxs_dnn_convolution_setup_bwd_block_H(handle);
  handle->use_fallback_bwd_loops = libxs_dnn_convolution_setup_fallback_loops_bwd(handle);
  handle->bwd_flags = libxs_dnn_convolution_setup_init_bwd_gemm_flags(handle);

  if ( ((handle->target_archid == LIBXS_X86_AVX512_SPR) && (handle->target_archid <= LIBXS_X86_ALLFEAT)) && (handle->datatype_in == LIBXS_DNN_DATATYPE_BF16) ) {
    handle->block_bwd_ifm = 1;
    handle->block_bwd_oj = handle->bwd_ofh_rb ;
    ldx = ((libxs_blasint)handle->ofmblock);
    ldA = handle->ifmblock;
    ...
    if (handle->desc.R == 1 && handle->desc.S == 1) {
      size_t stride_a = handle->desc.R * handle->desc.S * handle->ifmblock * handle->ofmblock * libxs_dnn_typesize(handle->datatype_in);
      size_t stride_b = handle->ofwp * handle->ofhp * handle->ofmblock * libxs_dnn_typesize(handle->datatype_in);
      handle->bwd_compute_kernel_strd = libxs_bsmmdispatch_reducebatch_strd_unroll(handle->ifmblock, handle->bwd_gemm_pixels, handle->ofmblock,
        (libxs_blasint)stride_a, (libxs_blasint)stride_b, handle->blocksofm_blocking, &ldA, &ldx, &ldC, NULL, &beta, &l_flags, NULL);
    } else {
        ...
        for (kj = 0; kj < handle->desc.R; kj++) {
          for (ki = 0; ki < handle->desc.S; ki++) {
            ...
            handle->B_offsets_bwd[i] = (ofm * handle->ofhp * handle->ofwp * handle->ofmblock +
                ...
                ki * handle->ofmblock) * libxs_dnn_typesize(handle->datatype_in);
            i++;
          }
        }
      }
      handle->bwd_compute_kernel_offs = libxs_bsmmdispatch_reducebatch_offs(handle->ifmblock, handle->bwd_gemm_pixels, handle->ofmblock, &ldA, &ldx, &ldC, NULL, &beta, &l_flags, NULL);
    }
    handle->bwd_config_kernel = libxs_bsmmdispatch(handle->ifmblock, handle->bwd_gemm_pixels, handle->ofmblock, &ldA, &ldx, &ldC, NULL, &beta, &l_tc_flags, NULL);
  }
  ...
  handle->upd_avoid_rim_fmas = libxs_dnn_convolution_setup_avoid_rim_fmas_upd(handle);
  handle->upd_pack_input = libxs_dnn_convolution_setup_pack_input_upd(handle);
  handle->upd_use_batchreduce = libxs_dnn_convolution_setup_use_batchreduce_upd(handle);
  handle->upd_ofw_rb = libxs_dnn_convolution_setup_upd_ofw_rb(handle);
  handle->upd_ofh_rb = libxs_dnn_convolution_setup_upd_ofh_rb(handle);
  handle->upd_loop_order = libxs_dnn_convolution_setup_loop_order_upd(handle);
  handle->weight_copies = libxs_dnn_convolution_setup_weight_copies_upd(handle);
  handle->block_upd_ofm = libxs_dnn_convolution_setup_block_upd_OFM(handle);
  handle->block_upd_ifm = libxs_dnn_convolution_setup_block_upd_IFM(handle);
  handle->upd_loop_order = libxs_dnn_convolution_setup_loop_order_upd(handle);
  ...
  if (handle->datatype_in == LIBXS_DNN_DATATYPE_BF16) {
    if  ((handle->target_archid == LIBXS_X86_AVX512_SPR) && (handle->target_archid <= LIBXS_X86_ALLFEAT)) {
      libxs_dnn_convolution_setup_bf16_upd_amx(handle);
    } else {
  ...
  /* setup up scratch */
  libxs_dnn_convolution_setup_fwd_scratch( handle );
  libxs_dnn_convolution_setup_bwd_scratch( handle );
  libxs_dnn_convolution_setup_upd_scratch( handle );
  handle->scratch = 0;
  ...
  *status = LIBXS_DNN_SUCCESS;
  ...
  if (0 != handle) {
    ...
    if ( (conv_desc.datatype_in == LIBXS_DNN_DATATYPE_F32) && (conv_desc.datatype_out != LIBXS_DNN_DATATYPE_F32) ) {
      /* error */
    } else if ( (conv_desc.datatype_in == LIBXS_DNN_DATATYPE_BF16) && (conv_desc.datatype_out != LIBXS_DNN_DATATYPE_BF16) ) {
      /* error */
    ...
    *status = libxs_dnn_convolution_setup(handle);
  }
  else {
...
LIBXS_API libxs_dnn_err_t libxs_dnn_trans_reg_bf16_filter(const libxs_dnn_layer* handle) {
  ...
  if (handle != 0) {
    if ( (handle->reg_filter != 0) && (handle->reg_filter_tr != 0) ) {
      ...
      LIBXS_VLA_DECL(7, libxs_bfloat16, wt, (libxs_bfloat16*)handle->reg_filter->data, handle->blocksifm, handle->desc.R, handle->desc.S, ifmblock_lp, handle->ofmblock, lpb);
      LIBXS_VLA_DECL(7, libxs_bfloat16, tr_wt, (libxs_bfloat16*)handle->reg_filter_tr->data, handle->blocksofm, handle->desc.R, handle->desc.S, ofmblock_lp, handle->ifmblock, lpb);
            ...
            for (ki=0; ki < handle->desc.S; ++ki) {
              for ( ofm2 = 0; ofm2 < handle->ofmblock; ++ofm2 ) {
                for ( ifm2 = 0; ifm2 < handle->ifmblock; ++ifm2 ) {
                  LIBXS_VLA_ACCESS(7, tr_wt, ifm1, ofm1, handle->desc.R-1-kj , handle->desc.S-1-ki, ofm2/lpb, ifm2, ofm2%lpb, handle->blocksofm, handle->desc.R, handle->desc.S, ofmblock_lp, handle->ifmblock, lpb) =
                    LIBXS_VLA_ACCESS(7, wt, ofm1, ifm1, kj, ki, ifm2/lpb, ofm2, ifm2%lpb, handle->blocksifm, handle->desc.R, handle->desc.S, ifmblock_lp, handle->ofmblock, lpb);
                }
              }
            }
          }
  ...
  *status = LIBXS_DNN_SUCCESS;
  ...
  if (0 != handle) {
    switch (kind) {
      ...
      case LIBXS_DNN_COMPUTE_KIND_UPD: break;
      case LIBXS_DNN_COMPUTE_KIND_ALL: break;
      default: {
